...
class Session(object):
    ...
    def __init__(self, path=None):
        ...
        #: The Security Factory class has the function to instantiatee the Authenticator Interface,
    ...
    def urlfilter(self, input_string):
        """Prepare a string to be sent in a URI query field."""
        s = input_string.replace('%', '%25').replace('#', '%23').replace('?', '%2F')
        return s.replace('&', '%26').replace('+', '%2B')
    ...
    def post(self, url, data):
        ...
        if isinstance(data, dict):
            for key in data:
                s = s + '&' + key + '=' + self.urlfilter(data[key])
            s = '?' + s[1:]
        else
            s = '?' + data
